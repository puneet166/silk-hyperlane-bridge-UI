...
export function ChainSelectField({ name, label, onChange, disabled, customListItemField }: Props) {
  ...
  // Only allow "silk" and "tan"
  const allowedChains = ['silk', 'tan' , 'bsctestnet'];
  const selectedChain = allowedChains.includes(field.value) ? field.value : 'silk'; // Default to "silk"
  const displayName = useChainDisplayName(selectedChain, true);

  const handleChange = (chainName: ChainName) => {
    if (!allowedChains.includes(chainName)) return; // Prevent selecting other chains
      ...
      >
        <div className="flex items-center gap-3">
          <div className="max-w-[1.4rem] sm:max-w-fit">
            <ChainLogo chainName={selectedChain} size={32} />
          </div>
